...
; Each entry is 3 bytes of real Z80 code, written verbatim into hAntiCrashBuffer
; and executed there when RST $28/$30/$38 fires (see home/header.asm).
Menu4FixList:
	db $00,$00,$00             ; Turn off: do nothing (falls through to whatever follows in HRAM)
	db $c3, LOW(Start), HIGH(Start) ; Jump fix: JP Start -- clean soft-reset to a known-good state.
	                                 ; (previously jumped to $01B9, which isn't code in this
	                                 ; build -- it lands inside MapHeaderPointers, a data table)
	db $c9,$de,$ad             ; Return fix: RET (trailing bytes are never reached, "DEAD" marker)
	db $e1,$e1,$c9             ; Dbl-return fix: POP HL / POP HL / RET -- unwinds 2 stack entries,
	                                 ; then RET pops a 3rd and jumps to it (same net effect as the
	                                 ; old 4-byte POP/POP/POP/JP HL, in 3 bytes instead of 4)
	db $e1,$e1,$e9             ; Stack fix: POP HL / POP HL / JP HL

Menu4Perform:
	ld a, [wDebugMenuCursorPos] ; load cursor position
	ld b, a
	add a, a  ; a = pos*2
	add a, b  ; a = pos*2 + pos = pos*3 (each entry is 3 bytes)
	ld b,$00
	ld c,a
	ld hl,Menu4FixList
	add hl,bc
	ld de,hAntiCrashBuffer
	ld c,$03
